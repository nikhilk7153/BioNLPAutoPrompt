...
llama = dspy.HFClientVLLM(model="meta-llama/Meta-Llama-3-8B", port=8000, url="http://gpua037.delta.ncsa.illinois.edu", tokens=1000)
...
#random.shuffle(train_dspy_examples)
#val_dspy_examples = random.sample(val_dspy_examples, 300)

config = dict(max_bootstrapped_demos=5, max_labeled_demos=0, num_candidate_programs=10, num_threads=6)
teleprompter = BootstrapFewShotWithRandomSearch(metric = eval_metric, **config)
optimized = teleprompter.compile(student = llama3, teacher = llama3, trainset=train_dspy_examples, valset=val_dspy_examples)

evaluate_test(optimized)

teacher = dspy.AzureOpenAI(api_base='", api_version='', model='gpt-4')

tqdm._instances.clear()

random.seed(42)

train_dspy_examples = random.sample(train_dspy_examples, 1000)
val_dspy_examples = random.sample(val_dspy_examples, 300)

teleprompter = MIPROv2(prompt_model= azure_turbo, task_model=teacher, metric=eval_metric, num_candidates=10, metric_threshold=None)
kwargs = dict(num_threads=3, display_progress=True, display_table=True)
compiled_prompt_opt = teleprompter.compile(MedQA(), trainset=train_dspy_examples, valset= val_dspy_examples, max_labeled_demos=0, max_bootstrapped_demos=5, eval_kwargs=kwargs, requires_permission_to_run=False)
'''